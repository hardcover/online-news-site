...
$button = '      <p><input type="submit" class="left" name="addUpdate" value="Add / update"/><input type="submit" name="reset" class="right" value="Reset" /></p>' . "\n";
if (isset($idAdPost)) {
    $_POST['edit'] = 1;
}
//
...
if (isset($_POST['addUpdate'])) {
    ...
    //
    $dbh = new PDO($dbClassifiedsNew);
    $stmt = $dbh->prepare('UPDATE ads SET email=?, title=?, description=?, categoryId=?, review=? WHERE idAd=?');
    $stmt->execute(array($email, $titlePost, $descriptionPost, $categoryIdPost, time(), $idAdPost));
    $dbh = null;
    //
...
if (isset($_POST['deleteApproved']) and isset($idAdPost)) {
    ...
    $stmt->execute(array(null, $idAdPost));
    $dbh = null;
}
//
// Button: Edit
...
if (isset($_POST['edit']) and isset($idAdPost)) {
    $dbh = new PDO($dbClassifiedsNew);
    $stmt = $dbh->prepare('SELECT title, description, categoryId, photo1 FROM ads WHERE idAd=?');
    $stmt->setFetchMode(PDO::FETCH_ASSOC);
    $stmt->execute(array($idAdPost));
    $row = $stmt->fetch();
    if ($row) {
        ...
        if (!is_null($row['photo1'])) {
            $button = '      <p><input type="submit" class="left" name="addUpdate" value="Add / update"/><input type="submit" class="middle" name="photoDelete" value="Delete photos" /><input type="submit" name="reset" class="right" value="Reset" /></p>'. "\n";
        }
    }
    $dbh = null;
}
...
foreach ($stmt as $row) {
    ...
    echo '      ' . $description . '<br />' . "\n";
    echo '      <input type="submit" class="left" name="edit" value="Edit" /><input type="submit" class="right" name="deletePending" value="Delete" /></p>' . "\n";
    echo '    </form>' . "\n\n";
}
$dbh = null;
//
