...
if (password_verify($adminPassPost, $row['pass'])) {
    if (password_needs_rehash($row['pass'], PASSWORD_DEFAULT)) {
        ...
        $dbh = new PDO($dbEditors);
        $stmt = $dbh->prepare('UPDATE sections SET pass=? WHERE idSection=?');
        $stmt->execute(array($newHash, $row['idSection']));
        $dbh = null;
    }
    //
    ...
    if (isset($_POST['addUpdate'])) {
        ...
        if ($sectionPost != null) {
            if ($parentSectionPost == 0) {
                ...
                $row = $stmt->fetch();
                if ($row and $sortOrderSectionPost > $row['sortOrderSection']) {
                ...
                $stmt->execute(array($idSection));
                $row = $stmt->fetch();
                if ($row and $sortOrderSectionPost > $row['sortOrderSubsection']) {
                    $sortOrderSectionPost++;
                }
...
?>
  ...
  <h4 class="m"><a class="m" href="menu.php">&nbsp;Menu&nbsp;</a><a class="m" href="settings.php">&nbsp;Settings&nbsp;</a><a class="s" href="classifiedSections.php">&nbsp;Classifieds&nbsp;</a></h4>
