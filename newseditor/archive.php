    ...
    if (isset($bylinePost) or isset($headlinePost) or isset($startDatePost) or isset($textPost)) {
        ...
        foreach ($stmt as $row) {
            ...
            if (isset($editorView) and $editorView == 1) {
                $html.= "\n" . '  <form action="' . $uri . 'archive.php" method="post">' . "\n";
                $html.= '    <p> <input type="hidden" name="idArticle" value="' . $idArticle . '"><input type="submit" value="Delete" name="delete" class="left" /><input type="submit" value="Return to edit" name="edit" class="right" /></p>' . "\n";
                $html.= "  </form>\n";
            }
