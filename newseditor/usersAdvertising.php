<?php
/**
 * User maintenance for the users that maintain advertisements
 *
 * PHP version 5
 *
 * @category  Publishing
...
$rowcount = null;
$dbh = new PDO($dbEditors);
$stmt = $dbh->query('SELECT idUser, user, pass, fullName FROM users WHERE userType = 3 ORDER BY fullName');
$stmt->setFetchMode(PDO::FETCH_ASSOC);
foreach ($stmt as $row) {
    extract($row);
    if (empty($pass)) {
