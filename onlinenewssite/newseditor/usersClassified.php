<?php
/**
 * User maintenance for the users who maintain classified ads
 *
 * PHP version 8
 *
 * @category  Publishing
    ...
    if (isset($_POST['addUpdate'])) {
        ...
        if (isset($_POST['user']) and isset($idUser)) {
            ...
            if (is_null($hash)) {
                ...
                $stmt->execute([$userPost, $fullNamePost, 4, $idUser]);
            } else {
                $stmt = $dbh->prepare('UPDATE users SET user=?, pass=?, fullName=?, userType=? WHERE idUser=?');
                $stmt->execute([$userPost, $hash, $fullNamePost, 4, $idUser]);
            }
            $dbh = null;
        } else {
            if (empty($message)) {
...
echo "  <title>Classifieds user maintenance</title>\n";
echo '  <script src="z/wait.js"></script>' . "\n";
require $includesPath . '/header2.inc';
?>

  <nav class="n">
    <h4 class="m"><a class="m" href="usersEditors.php">Editing users</a> <a class="m" href="usersSubscribers.php">Patron mgt users</a> <a class="m" href="usersAdvertising.php">Advertising users</a> <a class="s" href="usersClassified.php">Classified users</a> <a class="m" href="usersMenu.php">Menu users</a> <a class="m" href="settings.php">Settings</a> <a class="m" href="classifiedSections.php">Classifieds</a></h4>
  </nav>
<?php echoIfMessage($message); ?>
  ...
  <div class="flex">
    <main>
      <h1>Classifieds user maintenance</h1>

      <form class="wait" action="<?php echo $uri; ?>usersClassified.php" method="post">
    ...
    <aside>
      <h1>Classifieds users</h1>

<?php
$dbh = new PDO($dbEditors);
$rowcount = null;
$stmt = $dbh->query('SELECT idUser, user, pass, fullName FROM users WHERE userType = 4 ORDER BY fullName');
$stmt->setFetchMode(PDO::FETCH_ASSOC);
foreach ($stmt as $row) {
    extract($row);
    if (empty($pass)) {
    ...
    if ($user !== 'admin') {
        $rowcount++;
        echo '      <form class="wait" action="' . $uri . 'usersClassified.php" method="post">' . "\n";
