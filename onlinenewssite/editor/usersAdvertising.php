    ...
    if (isset($_POST['addUpdate'])) {
        ...
        if (isset($_POST['user']) and isset($idUser)) {
            ...
            if (is_null($hash)) {
                ...
                $stmt->execute([$userPost, $fullNamePost, 3, $idUser]);
            } else {
                $stmt = $dbh->prepare('UPDATE users SET user=?, pass=?, fullName=?, userType=? WHERE idUser=?');
                $stmt->execute([$userPost, $hash, $fullNamePost, 3, $idUser]);
            }
            $dbh = null;
        } else {
            if (empty($message)) {
...
<?php echoIfMessage($message); ?>
  ...
  <div class="flex">
    <main>
      ...
      <form action="<?php echo $uri; ?>usersAdvertising.php" method="post">
...
foreach ($stmt as $row) {
    ...
    if ($user !== 'admin') {
        $rowcount++;
        echo '      <form action="' . $uri . 'usersAdvertising.php" method="post">' . "\n";
