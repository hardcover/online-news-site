...
echo "  <title>Subscriber user maintenance</title>\n";
echo '  <script src="z/wait.js"></script>' . "\n";
require $includesPath . '/editor/header2.inc';
?>

  <nav class="n">
    <h4 class="m"><a class="m" href="usersEditors.php">Editing users</a> <a class="s" href="usersSubscribers.php">Patron mgt users</a> <a class="m" href="usersAdvertising.php">Advertising users</a> <a class="m" href="usersClassified.php">Classified users</a> <a class="m" href="usersMenu.php">Menu users</a> <a class="m" href="settings.php">Settings</a> <a class="m" href="classifiedSections.php">Classifieds</a></h4>
  </nav>
<?php echoIfMessage($message); ?>

  <div class="flex">
    <main>
      <h1>Subscriber management user maintenance</h1>

      <form action="<?php echo $uri; ?>usersSubscribers.php" method="post">
        ...
        <p><input type="submit" value="Add / update" name="addUpdate" class="button"> <input type="submit" class="button" value="Delete" name="delete"><input type="hidden" name="existing"<?php echoIfValue($edit); ?>></p>
      </form>
    </main>

    <aside>
      <h2>Subscriber management users</h2>

<?php
$rowcount = null;
$dbh = new PDO($dbEditors);
$stmt = $dbh->query('SELECT idUser, user, pass, fullName FROM users WHERE userType = 2 ORDER BY fullName');
$stmt->setFetchMode(PDO::FETCH_ASSOC);
$stmt->execute();
foreach ($stmt as $row) {
    extract($row);
    ...
    if ($user !== 'admin') {
        $rowcount++;
        echo '      <form action="' . $uri . 'usersSubscribers.php" method="post">' . "\n";
