...
if (isset($_POST['update']) and isset($idAdPost)) {
    ...
    if (isset($titlePost) and isset($descriptionPost) and isset($categoryIdPost) and isset($startDatePost) and isset($durationPost)) {
        ...
        $dbh = new PDO($dbClassifieds);
        $stmt = $dbh->prepare('UPDATE ads SET title=?, description=?, categoryId=?, endDate=?, startDate=?, duration=?, invoice=?, photos=? WHERE idAd=?');
        $stmt->execute([$titlePost, $descriptionPost, $categoryIdPost, $endDate, $startDatePost, $durationPost, $invoicePost, $photosPublished, $idAdPost]);
        $dbh = null;
...
if (isset($_POST['edit']) and isset($_POST['idAd'])) {
    $dbh = new PDO($dbClassifieds);
    $stmt = $dbh->prepare('SELECT idAd, email, title, description, categoryId, endDate, startDate, duration, invoice FROM ads WHERE idAd=?');
    $stmt->setFetchMode(PDO::FETCH_ASSOC);
    $stmt->execute([$idAdPost]);
    $row = $stmt->fetch();
    $dbh = null;
...
foreach ($searchResults as $searchResult) {
    ...
    echo '          ' . Parsedown::instance()->parse($searchResult['description']) . " - Description<br>\n";
